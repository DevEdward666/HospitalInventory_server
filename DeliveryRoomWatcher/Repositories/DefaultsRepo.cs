...
namespace DeliveryRoomWatcher.Repositories
{
    public class DefaultsRepo
    {
        ...
        public ResponseModel getnotications(mdlNotifications.notifbydept notifications)
        {
            using (var con = new MySqlConnection(DatabaseConfig.GetConnection()))
            {
                con.Open();
                using (var tran = con.BeginTransaction())
                {
                    try
                    {
                        var data = con.Query($@"SELECT * FROM requestnotification WHERE audience='all' OR audience=@dept ORDER BY createdAt DESC LIMIT @offset", notifications, transaction: tran);
        ...
        public ResponseModel getnoticationsAdmin(mdlNotifications notifications)
        {
            using (var con = new MySqlConnection(DatabaseConfig.GetConnection()))
            {
                ...
                using (var tran = con.BeginTransaction())
                {
                    try
                    {
                        var data = con.Query($@"SELECT * FROM requestnotification", notifications, transaction: tran);
        ...
        public ResponseModel getnoticationsAdmin(mdlNotifications.searchNotif notifications)
        {
            using (var con = new MySqlConnection(DatabaseConfig.GetConnection()))
            {
                ...
                using (var tran = con.BeginTransaction())
                {
                    try
                    {
                        var data = con.Query($@"SELECT * FROM requestnotification where title LIKE concat('%',@title,'%') or priority=@priority ORDER BY createdAt DESC LIMIT @offset  ", notifications, transaction: tran);
            ...
            using (var con = new MySqlConnection(DatabaseConfig.GetConnection()))
            {
                ...
                using (var tran = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        int insert_user_otp = con.Execute($@"INSERT INTO requestnotification (title,body,priority,audience,createdBy) values(@title,@body,@priority,@audience,@created_by) ",
